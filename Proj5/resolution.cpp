...
/********************************
 * function resolvable():
 * Helper function for seeing if 2 clauses share
 * a common literal but with opposite signs
 * Arguments:
 * 1: clause 1
 * 2: clause 2
 * *****************************/
    ...
    for (i = 0; i < KB.size(); i++) {
        for (j = i+1; j < KB.size(); j++) {
            if (resolvable(KB[i], KB[j])) {
                // get # of literals of 2 clauses, create ResPair object
                int len;
